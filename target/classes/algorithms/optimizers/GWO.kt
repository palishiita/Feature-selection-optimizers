...
class GWO(
    ...
    override fun optimize(dataset: DataFrame<*>, fitnessFunction: FitnessFunction): DataFrame<*> {
        ...
        var wolves =
            List(populationSize) { List(numFeatures) { if (Random.nextDouble() > 0.5) 1 else 0 } }
        ...
        println(
            "Starting $name with a budget of $maxSolutions evaluations (Population: $populationSize)."
        )
        ...
        while (totalEvaluations < maxSolutions) {
            val results: List<FitnessResult> =
                wolves.map { fitnessFunction.evaluateDetailed(dataset, it) }
            val fitnesses = results.map { it.fitness }
            val maxFitnessIter = fitnesses.maxOrNull() ?: Double.NEGATIVE_INFINITY
            val minFitnessIter = fitnesses.minOrNull() ?: Double.POSITIVE_INFINITY
            val avgFitnessIter = if (fitnesses.isNotEmpty()) fitnesses.average() else 0.0
            ...
            val a = max(2.0 * (1 - iter.toDouble() / maxIterations), minA)
            wolves =
                wolves.map { wolf ->
                    List(numFeatures) { i ->
            ...
            if (iter >= maxIterations) {
                println(
                    "Warning: Exiting due to reaching the failsafe iteration limit ($maxIterations)."
                )
                break
            }
        }
